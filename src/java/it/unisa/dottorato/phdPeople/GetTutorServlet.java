 ...
 */
@WebServlet(name = "GetTutor", urlPatterns = {"/dottorato/GetTutor"})
public class GetTutorServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            String aStudent = request.getParameter("SSN");
            JSONObject result = new JSONObject();
            try {
                Person aPerson = PhdPeopleManager.getInstance().getTutor(aStudent);
                result.put("ssn", aPerson.getSsn());
